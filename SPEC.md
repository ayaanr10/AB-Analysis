...
**Status:** v1 built. All six phases complete; every §12 question resolved.
**Owner:** Shreyas Jain
**Type:** Portfolio project (personal, not commercial, no client or employer involved)
**Last updated:** 28 July 2026

> **Where the built system diverges from this document.** Four places, all recorded with
> reasoning in [`docs/decisions.md`](docs/decisions.md). This spec is left as written so the
> divergences stay visible rather than being edited away.
>
> | Divergence | Why | Where |
> |---|---|---|
> | Pre-period balance judged on standardised mean difference, not p-values | At n = 14M all 12 Criteo covariates reject on p while sitting inside \|SMD\| = 0.05. A p-value implementation blocks a usable experiment. | D-04 |
> | Metric horizon carried in `event_type`, not one shared `return` event | §6 says map each true flag to a synthetic `return` event. Taken literally, day-1 and day-7 become indistinguishable once `event_at` is null. | `adapters/cookie_cats.py` |
> | Power computed per horizon, not once per experiment | Cookie Cats' baselines are 44.8% and 19.0%; one MDE misdescribes both. This is what reveals that day-1 could not have detected the effect that existed. | D-09 |
> | The gate has a fourth state, `NOT_APPLICABLE` | Criteo has no unit id, so duplicate detection cannot fail. A check that cannot go red must not report as a pass. | D-08, D-10 |
...
3. ~~**Is Tableau the right surface for the scorecard**~~ **RESOLVED, Phase 6 — generated
   HTML** (`readout/html.py`), and the reason is the blocking rule. §7.3 requires
   suppression in *every* output surface; in generated HTML that is one branch sharing the
   gate with every other surface, and a test asserts it. A Tableau workbook reads an extract,
   so it can go stale against the pipeline silently and its suppression would be a
   calculated field someone can unhide.

   The Tableau argument was a portfolio one and it was real: a second artifact on an
   existing Tableau Public profile. That is a genuine loss, recorded in `docs/decisions.md`
   D-11 rather than glossed. If the portfolio consideration wins later, the fix is a
   workbook *alongside* this reading the same DuckDB output, not replacing it.

   Both scorecards are generated by the same code with no edits:
   `report/cookie_cats_scorecard.html`, `report/criteo_scorecard.html`.

4. ~~**How far to take CUPED**~~ **RESOLVED, Phase 5 — a demonstration with reported
   variance reduction**, not a full treatment of pre-period selection: Criteo has no time
   dimension, so there is no pre-period to select from.

   Measured on `f9`: **variance reduction 22.33%** against a theoretical ceiling of
   corr² = 24.53%.

   The unplanned result is more interesting than the planned one. CUPED moved the point
   estimate by 24% of its size (+0.010342 to +0.007859), which is not a bug: `f9` is
   imbalanced across arms at |SMD| = 0.024 — inside the 0.10 threshold, reported as PASS by
   this system's own balance check — and the shift equals −theta × the covariate imbalance
   exactly. So a covariate that passes the balance check can still materially inflate a
   headline effect. See `docs/decisions.md` D-14 and `docs/case_study_2.md` §7.

5. ~~**Repository naming.**~~ **RESOLVED — `ab-test-readout`.** Published at
   github.com/shreyj13/ab-test-readout. "System" was dropped as filler; "ab-test" is the
   phrase a reader actually scans for, and "experiment" alone risks reading as "me
   experimenting" rather than "controlled experiments".
